...
class Units {
    ...
    render(canvas, size, dist, light, intensity, shadows = false, perspective = 1) {
        ...
        units.forEach(unit => {         
            const faces = unit.faces.sort((a, b) => Math.max(...b.map(point => point.z)) - Math.max(...a.map(point => point.z)));
            ...
            faces.forEach(face => {
                ...
                const brightness = shadows ? Math.min(1, Math.max(0, Vector.reflect(normal, light).prod)) : 1;
